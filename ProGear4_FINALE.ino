...
#define BATT_CHARGING   -1      // -1 = غير موصل

/* [OPTION] لو لاقطة الشحن تعطي HIGH وهو يشحن خليها 1، لو العكس 0 */
#define BATT_CHG_ACTIVE_HIGH 1

/* ================== Calibration & Limits ================== */
static float CAL_A = 210.0f;   // قناة A (Gain=128) — غيّريها بعد المعايرة
static float CAL_B = 210.0f;   // قناة B (Gain=32)  — غيّريها بعد المعايرة

static const float   MAX_G      = 10000.0f;
static const uint32_t SEND_MS   = 500;     // كل كم ميلي ثانية نرسل WEIGHT_DATA
static const float   QUIET_EPS  = 20.0f;   // فرق الغرامات لاعتبار القراءة ثابتة
static const uint32_t QUIET_MS  = 2500;    // كم نبقى ثابتين قبل SESSION_FINAL

/* ======================= Battery ======================= */
#define DIV_FACTOR      2.0f    // مقسم الجهد 100k/100k
#define SAMPLES         16
static float emaV = NAN;
static const float alpha = 0.20f;  // EMA تنعيم

// حالة البطارية السابقة (لمنع السبام)
static int lastBatPct      = -1;   // آخر نسبة أرسلناها
static int lastChg         = -1;   // آخر حالة شحن (0/1)
...
/* [COOLDOWN] تبريد رسائل الأخطاء/الحالات */
...
static void maybeSendBattery(uint32_t now, bool force=false){
  ...
  const int CHANGE_THRESHOLD      = 2;        // تجاهل تغيير أقل من ±2%
  const uint32_t MIN_INTERVAL_MS  = 30000;    // أقل شي 30 ثانية بين إرسالين

  bool first       = (lastBatPct < 0 || lastChg < 0);
  bool pctChanged  = (lastBatPct >= 0) ? (abs(p - lastBatPct) >= CHANGE_THRESHOLD) : true;
  bool chgChanged  = (lastChg >= 0) ? (chg != lastChg) : true;
  bool timeOk      = (now - lastBatSentMs >= MIN_INTERVAL_MS);

  // نرسل في الحالات المهمة فقط (أول مرة، تغيّر ملحوظ، تغيّر حالة الشحن) + احترام الفاصل الزمني
  if (force || first || pctChanged || chgChanged) {
    if (force || timeOk) {
...
static long readChannelRaw(byte gain, uint8_t samples) {
  scale.set_gain(gain);
  delay(80);                  // تأخير كافي بعد تغيير الـ gain
...
class RxCB : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* c) override {
    ...
    while ((idx = rxBuf.indexOf('\n')) >= 0) {
      ...
      if (U == "PING") {
        sendLine("STATUS_UPDATE", "{\"state\":\"pong\"}");
      }
        ...
        maybeSendBattery(millis(), /*force=*/true);  // رد فوري
      }
      else if (U == "GET_WEIGHT") {                  // اختياري للتشخيص
...
void loop() {
  ...
  if (now - lastSend >= SEND_MS) {
    ...
    if (g < 0) {
      ...
      if (g > MAX_G) {
        ...
        // أرسل WEIGHT_DATA لو تغيّر ≥1g أو أول مرة
        if (lastG < 0 || fabsf(g - lastG) >= 1.0f) {
          sendWeight(g);
        }

        // منطق الثبات → SESSION_FINAL مرة وحدة
  ...
  // بطارية (مع منع السبام)
  maybeSendBattery(now, /*force=*/false);
}
